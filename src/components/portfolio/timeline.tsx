    ...
    <section id="journey" className="relative py-28 sm:py-36">
      ...
      <div className="site-container relative">
        ...
        <div ref={timelineRef} className="relative mt-16 sm:mt-20">
          ...
          <motion.div
            aria-hidden="true"
            className="absolute top-0 bottom-0 left-2 w-px origin-top bg-gradient-to-b from-highlight via-indigo-400 to-secondary shadow-[0_0_18px_rgba(167,139,250,0.7)] md:left-1/2"
            style={{ scaleY: reduceMotion ? 1 : smoothProgress }}
          />
          <motion.div
            aria-hidden="true"
            className="absolute left-2 z-10 size-3 -translate-x-[5.5px] rounded-full bg-highlight shadow-[0_0_0_6px_rgba(167,139,250,0.12),0_0_28px_rgba(167,139,250,0.95)] md:left-1/2"
            style={{ top: reduceMotion ? "calc(100% - 0.75rem)" : dotPosition }}
          />
            ...
            {timeline.map((item, index) => {
              ...
              return (
                <li key={item.year} className="relative pl-11 md:grid md:grid-cols-2 md:gap-20 md:pl-0">
                  <span
                    aria-hidden="true"
                    className="absolute top-7 left-2 size-3 -translate-x-[5.5px] rounded-full border-2 border-highlight bg-background md:left-1/2"
                  />
                  <motion.article
                    className={`glass-card rounded-[1.65rem] p-6 sm:p-8 ${
                  ...
                  >
                    <span className="inline-flex rounded-full bg-gradient-to-r from-primary to-indigo-400 px-3.5 py-1.5 text-xs font-semibold text-white shadow-[0_8px_24px_rgba(167,139,250,0.18)]">
                      {item.year}
                    </span>
