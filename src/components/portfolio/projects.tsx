  ...
  return (
    <div className="flex flex-wrap gap-3">
      {project.githubUrl && (
        ...
        >
          <FiGithub size={16} aria-hidden="true" /> Repository <ArrowUpRight className="button-arrow" size={14} aria-hidden="true" />
        </a>
      )}
      {project.liveUrl && (
        <a
...
export function Projects() {
  const stickyContent: StickyScrollItem[] = projects.map((project, index) => ({
    ...
    details: (
      <>
        <p className="mt-3 text-[0.68rem] tracking-[0.19em] text-highlight uppercase">{project.category}</p>
        <p className="mt-5 text-base font-medium text-zinc-300">{project.subtitle}</p>
    ...
    content: <ProjectVisual project={project} index={index} />,
  }));
    ...
    <section id="projects" className="relative">
      ...
      <div className="site-container hidden py-24 lg:block">
        <SectionHeading
          eyebrow="Projects"
          title="Selected Projects"
          description="Scroll through full-stack, mobile, and academic projects. The preview stays in place and changes as each project comes into focus."
        />
